export class GameDiff {
	async ParseGameData(base_game_data, item_data, debugEnabled = false, verbose = false) {
		let baseGame = this.InitialiseGame(base_game_data)
		let modifiedGame = this.InitialiseGame(base_game_data)
		// console.log("Initialised modifiedGame", modifiedGame)

		let diffGame = new SimGame();
		[modifiedGame, diffGame] = this.ParseDataSet(modifiedGame, item_data, diffGame, baseGame)
		...
		const data = this.CreateFilteredList(baseGame, modifiedGame, diffGame)
	...
	ParseDataSet(game, data, diffGame, baseGame) {
		Object.entries(data).forEach(([category, category_data]) => {
			switch (category) {
				case "modifications":
					[game, diffGame] = this.ParseModificationData(game, category_data, diffGame, baseGame)
					break;
				case "dependentData": category_data.forEach(dependent_data => {
					switch (dependent_data.namespace) {
						case "melvorF":
							if (cloudManager.hasFullVersionEntitlement) [game, diffGame] = this.ParseDataSet(game, dependent_data, diffGame, baseGame)
							break;
						case "melvorTotH":
							if (cloudManager.hasTotHEntitlementAndIsEnabled) [game, diffGame] = this.ParseDataSet(game, dependent_data, diffGame, baseGame)
							break;
						case "melvorAoD":
							if (cloudManager.hasAoDEntitlementAndIsEnabled) [game, diffGame] = this.ParseDataSet(game, dependent_data, diffGame, baseGame)
							break;
						case "melvorItA":
							if (cloudManager.hasItAEntitlementAndIsEnabled) [game, diffGame] = this.ParseDataSet(game, dependent_data, diffGame, baseGame)
							break;
						default:
							// if (debugEnabled)
	...
	ParseModificationData(game, modification_data, diffGame, baseGame) {
		...
		function ParseMonsterData(game, data, diffGame) { return ParseCategoryData(game, data, diffGame, "monsters", "lootTable"); }
		function ParseItemData(game, data, diffGame, item_id) { return ParseCategoryData(game, [{ id: item_id, lootTable: data }], diffGame, "items", "dropTable"); }
	...
	CreateFilteredList(baseGame, modifiedGame, diffGame) {
